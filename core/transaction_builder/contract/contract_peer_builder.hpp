...
*/
#ifndef CORE_MODEL_TRANSACTION_BUILDER_CONTRACT_PEER_HPP
#define CORE_MODEL_TRANSACTION_BUILDER_CONTRACT_PEER_HPP

#include <infra/protobuf/api.pb.h>
#include <util/exception.hpp>
#include "../transaction_builder_base.hpp"
#include "../type_signatures/commands/contract.hpp"
#include "../type_signatures/objects.hpp"

namespace txbuilder {

template <>
class TransactionBuilder<type_signatures::Contract<type_signatures::Peer>> {
 public:
  ...
  TransactionBuilder& setPeer(Api::Peer object) {
    if (_isSetPeer) {
      throw exception::txbuilder::DuplicateSetArgmentException("Contract<Peer>",
                                                               "Peer");
    }
    _isSetPeer = true;
  ...
  Api::Transaction build() {
    ...
    if (not unsetMembers.empty()) {
      throw exception::txbuilder::UnsetBuildArgmentsException("Contract<Peer>",
                                                              unsetMembers);
    }
    Api::Transaction ret;
    ret.set_senderpubkey(_senderPublicKey);
    ret.set_type("Contract");
