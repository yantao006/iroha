...
template <>
class TransactionBuilder<type_signatures::Update<object::SimpleAsset>> {
 public:
  ...
  TransactionBuilder& setSender(std::string sender) {
    if (_isSetSender) {
      throw std::domain_error(
          ...
          "update/update_simple_asset_builder_template.hpp");
    }
    _isSetSender = true;
  ...
  transaction::Transaction build() {
    ...
    if (not unsetMembers.empty()) {
      throw exception::transaction::UnsetBuildArgmentsException(
          "Update<object::SimpleAsset>", unsetMembers);
    }
    return transaction::Transaction(_sender, command::Update(_simpleAsset));
  }
