...
*/
#ifndef CORE_MODEL_TRANSACTION_BUILDER_UPDATE_MESSAGE_HPP
#define CORE_MODEL_TRANSACTION_BUILDER_UPDATE_MESSAGE_HPP
...
template <>
class TransactionBuilder<type_signatures::Update<object::Message>> {
 public:
  ...
  TransactionBuilder& setSender(std::string sender) {
    if (_isSetSender) {
      throw std::domain_error(std::string("Duplicate sender in ") +
                              "update/update_message_builder_template.hpp");
    }
    _isSetSender = true;
  ...
  transaction::Transaction build() {
    ...
    if (not unsetMembers.empty()) {
      throw exception::transaction::UnsetBuildArgmentsException(
          "Update<object::Message>", unsetMembers);
    }
    return transaction::Transaction(_sender, command::Update(_message));
  }
