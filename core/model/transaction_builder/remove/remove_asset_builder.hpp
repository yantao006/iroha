...
template <>
class TransactionBuilder<type_signatures::Remove<object::Asset>> {
 public:
  ...
  transaction::Transaction build() {
    ...
    if (not unsetMembers.empty()) {
      throw exception::transaction::UnsetBuildArgmentsException(
          "Remove<object::Asset>", unsetMembers);
    }
    return transaction::Transaction(_sender, command::Remove(_asset));
  }
