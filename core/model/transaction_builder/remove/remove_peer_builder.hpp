...
template <>
class TransactionBuilder<type_signatures::Remove<object::Peer>> {
 public:
  ...
  TransactionBuilder& setSender(std::string sender) {
    if (_isSetSender) {
      throw std::domain_error(std::string("Duplicate sender in ") +
                              "remove/remove_peer_builder_template.hpp");
    }
    _isSetSender = true;
  ...
  transaction::Transaction build() {
    ...
    if (not unsetMembers.empty()) {
      throw exception::transaction::UnsetBuildArgmentsException(
          "Remove<object::Peer>", unsetMembers);
    }
    return transaction::Transaction(_sender, command::Remove(_peer));
  }
