...
template <>
class TransactionBuilder<type_signatures::Transfer<object::Account>> {
 public:
  ...
  transaction::Transaction build() {
    ...
    if (not unsetMembers.empty()) {
      throw exception::transaction::UnsetBuildArgmentsException(
          "Transfer<object::Account>", unsetMembers);
    }
    return transaction::Transaction(
        _sender, command::Transfer(_account, _receiverPublicKey));
  }
