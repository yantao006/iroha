...
template <>
class TransactionBuilder<type_signatures::Transfer<object::SimpleAsset>> {
 public:
  ...
  transaction::Transaction build() {
    ...
    if (not unsetMembers.empty()) {
      throw exception::transaction::UnsetBuildArgmentsException(
          "Transfer<object::SimpleAsset>", unsetMembers);
    }
    return transaction::Transaction(
        _sender, command::Transfer(_simpleAsset, _receiverPublicKey));
  }
